...
int uinput_init() {
    ...
    for (int i = 0; i < sizeof(ALL_KEYS) / sizeof(*ALL_KEYS); i++) {
        ioctl(fd, UI_SET_KEYBIT, ALL_KEYS[i]);
    }
...
Emitted handle_key(int fd, int keycode, int is_long_press) {
    ...
    if (base_keymap[keycode] == KEY_NUMMODE) {
        current_keymap =
            (current_keymap == KEYMAP_NUM) ? KEYMAP_BASE : KEYMAP_NUM;
        e.code = -1;
        return e;
    }
    ...
    default:
        e.code =
            (is_long_press && base_keymap_dbl_keys[keycode] != KEY_UNDEFINED)
            ...
            for (int j = 0; j < NCOLS; j++) {
                ...
                if (pressed[keycode] != val) {
                    ...
                    if (val) {
                        ...
                        if (current_pressed_keys >= MAX_PRESSED_KEYS) continue;
                        last_press_started[keycode] = millis();
                    } else {
                        current_pressed_keys -= 1;
                        int long_press =
                            millis() - last_press_started[keycode] > 300;
