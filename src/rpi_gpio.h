...
static inline int rpi_gpio_init(void) {
  ...
  if (mem_fd < 0)
    return -1;
  gpio_map = (uint32_t *)mmap(NULL, GPIO_PAGE_SIZE, PROT_READ | PROT_WRITE,
                              MAP_SHARED, mem_fd, 0);
  close(mem_fd);
...
static inline void rpi_gpio_set_pull(int gpio, int pud) {
  if (IS_BCM2711) {
    ...
    write2(PULLUPDN_OFFSET_2711, gpio,
           pud == PUD_UP     ? 1
           : pud == PUD_DOWN ? 2
                             : 0);
  } else {
...
static inline int rpi_gpio_get_fn(int gpio) { return read3(FSEL_OFFSET, gpio); }
...
int rpi_gpio_input(int gpio) { return read1(PINLEVEL_OFFSET, gpio); }

uint32_t rpi_gpio_input_all(void) { return gpio_map[PINLEVEL_OFFSET]; }
